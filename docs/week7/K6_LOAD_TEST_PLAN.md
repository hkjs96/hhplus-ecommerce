...
> **📝 2025-12-09 업데이트:** Step 13-14 피드백 반영
>
> **주요 변경사항:**
> 1. ✅ **Ramp-up 시나리오 추가** - 점진적 부하 증가 테스트 (현실적 트래픽 패턴)
> 2. ✅ **LoadTestDataInitializer Profile 분리** - local/test 환경에서만 실행
> 3. ✅ **userId 1 잔액 증가** - 1억원 → 200억원 (K6 테스트 중 잔액 부족 해결)
> 4. ✅ **Connection Pool 조정** - 200 → 100 (실제 모니터링 기반 재조정 필요)
>
...
- ~~CouponReservation.status = ISSUED~~ (Deprecated - 테이블 제거됨)
...
**💡 2025-12-09 변경:**
- CouponReservation 테이블 제거됨 (Redis Only 구조)
- 검증 항목: Redis Set + UserCoupon만 확인

---

#### 🆕 Scenario 4: Ramp-up 테스트 (점진적 부하 증가)

> **추가 날짜:** 2025-12-09 (피드백 반영)
...
**목적:**
- 실제 운영 환경의 점진적 트래픽 증가 패턴 반영
- Cold Start 문제 식별
- 시스템 회복력 테스트

**플로우:**
1. [Stage 1: Warm-up] 0명 → 20명 (10초)
   - Connection Pool 초기화
   - JVM Warm-up
   - Cache Warm-up

2. [Stage 2: Load] 20명 → 50명 (20초)
   - 실제 부하 증가
   - Bottleneck 탐지

3. [Stage 3: Cool-down] 50명 → 0명 (10초)
   - 시스템 회복력 테스트
   - Connection Pool 정리
   - Memory GC

**검증 항목:**
- Ramp-up 시 응답 시간: p95 < 1000ms (Extreme보다 빠를 것으로 예상)
- Cold Start 지연 시간
- 회복 후 정상 동작

**Extreme vs Ramp-up 비교:**
...
**기대 효과:**
- 현실적인 성능 지표 확보
- Bottleneck 조기 발견 (20명 → 50명 증가 시 급격한 지연 감지)
- Cold Start 문제 식별
- 회복력 검증 (50명 → 0명 후에도 응답 시간 정상)
