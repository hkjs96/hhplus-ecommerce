...
export default function (data) {
    ...
    // 시나리오 2: 주문 + 결제 플로우 (30% 트래픽)
    if (random < 0.8) { // 0.5 + 0.3 = 0.8
...
function testOrderAndPaymentFlow(headers) {
    ...
    const orderPayload = JSON.stringify({
        ...
        couponId: null,
        idempotencyKey: `order-${Date.now()}-${__VU}-${__ITER}`
    });
    ...
    if (!paymentSuccessCheck) {
        ...
        errorRate.add(1); // Count any failure in payment check as an error for custom metric
        console.error(`Payment processing failed: ${paymentResponse.status} - ${paymentResponse.body}`);
        return;
    }
    ...
    if (paymentResponse.status === 200) {
        ...
        errorRate.add(0);
    } else { // Handle 400 as a business error, not system error
        paymentFailure.add(1);
        errorRate.add(0);
        console.warn(`Payment processing business error: ${paymentResponse.status} - ${paymentResponse.body}`);
    }
}
...
function testCouponIssuance(headers) {
    ...
    if (!success) {
        errorRate.add(1); // Still count as an error for custom metric
...
// 상품 랭킹 조회 테스트
function testProductRanking(headers) {
    const start = new Date().getTime();

    const response = http.get(`${BASE_URL}/api/products/ranking/top`, {
        headers: headers,
        tags: { name: 'GetProductRanking' },
    });

    const duration = new Date().getTime() - start;
    rankingDuration.add(duration);

    const success = check(response, {
        'Product ranking status is 200': (r) => r.status === 200,
        'Product ranking has data': (r) => {
            try {
                const body = JSON.parse(r.body);
                return body.rankings && Array.isArray(body.rankings);
            } catch (e) {
                return false;
            }
        },
    ...
    if (!success) {
        errorRate.add(1);
        console.error(`Product ranking failed: ${response.status} - ${response.body}`);
    } else {
        errorRate.add(0);
    }
}
