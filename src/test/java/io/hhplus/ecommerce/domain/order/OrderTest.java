...
class OrderTest {
    ...
    void create_성공() {
        ...
        // When
        Order order = Order.create("ORDER-001", testUser, subtotal, discount);
    ...
    void create_할인금액초과_예외발생() {
        // Given
        Long subtotal = 100000L;
        Long discount = 200000L;

        // When & Then
        assertThatThrownBy(() -> Order.create("ORDER-001", testUser, subtotal, discount))
    ...
    void complete_성공() {
        // Given
        Order order = Order.create("ORDER-001", testUser, 100000L, 10000L);
        assertThat(order.getStatus()).isEqualTo(OrderStatus.PENDING);
        assertThat(order.getPaidAt()).isNull();
    ...
    void 상태확인메서드() {
        // Given
        Order pendingOrder = Order.create("ORDER-001", testUser, 100000L, 10000L);
