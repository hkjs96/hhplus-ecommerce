...
class CouponControllerIntegrationTest {
    ...
    void issueCoupon_성공() throws Exception {
        // Given
        IssueCouponRequest request = new IssueCouponRequest("U001");

        // When & Then
        mockMvc.perform(post("/api/coupons/C001/issue")
    ...
    void issueCoupon_실패_존재하지않는사용자() throws Exception {
        // Given
        IssueCouponRequest request = new IssueCouponRequest("INVALID_USER");
    ...
    void issueCoupon_실패_존재하지않는쿠폰() throws Exception {
        ...
        // When & Then
        mockMvc.perform(post("/api/coupons/INVALID_COUPON/issue")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(request)))
    ...
    void issueCoupon_실패_만료된쿠폰() throws Exception {
        ...
        // When & Then
        mockMvc.perform(post("/api/coupons/C003/issue")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(request)))
                .andExpect(status().isBadRequest())
    ...
    void issueCoupon_실패_수량소진() throws Exception {
        ...
        IssueCouponRequest request1 = new IssueCouponRequest("U001");
        mockMvc.perform(post("/api/coupons/C999/issue")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(request1)))
                .andExpect(status().isOk());

        // When & Then - Second user fails
        IssueCouponRequest request2 = new IssueCouponRequest("U002");
        mockMvc.perform(post("/api/coupons/C999/issue")
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    void getUserCoupons_성공() throws Exception {
        ...
        LocalDateTime now = LocalDateTime.now();
        UserCoupon userCoupon1 = UserCoupon.create("UC001", "U001", "C001", now.plusDays(7));
        UserCoupon userCoupon2 = UserCoupon.create("UC002", "U001", "C002", now.plusDays(14));

        userCouponRepository.save(userCoupon1);
        userCouponRepository.save(userCoupon2);

        // When & Then
        mockMvc.perform(get("/api/users/U001/coupons"))
                .andExpect(status().isOk())
    ...
    void getUserCoupons_상태필터() throws Exception {
        ...
        mockMvc.perform(get("/api/users/U001/coupons")
                ...
                .andExpect(jsonPath("$.coupons[0].userCouponId").value("UC001"));
    }

    @Test
    @DisplayName("보유 쿠폰 조회 API - 존재하지 않는 사용자")
    void getUserCoupons_실패_존재하지않는사용자() throws Exception {
        mockMvc.perform(get("/api/users/INVALID_USER/coupons"))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.code").value("U001"));
    }
